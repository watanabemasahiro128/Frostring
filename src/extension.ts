...
export class Frostring implements vscode.CodeActionProvider {
  ...
  public provideCodeActions(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction[] | undefined {
    ...
    if (currentLine !== 0 || this.hasFrozenStringLiteralCommnet(document, currentLine)) {
      return;
    }
  ...
  private hasFrozenStringLiteralCommnet(document: vscode.TextDocument, currentLine: number) {
    const lineOfText = document.lineAt(currentLine);
    return lineOfText.text.includes(FROZEN_STRING_LITERAL);
  }
